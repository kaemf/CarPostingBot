...
import TRUSTED_USERS from './trusted';

export default async function arch() {
  const [ bot, db] = await init();
  ...
  bot.on('message', async (ctx, next) => {
    ...
    if (TRUSTED_USERS(user.id)) {
      ...
      console.log('(UNAUTHORIZED) [@', user['username'], '], ', '( id:', id, ')', '( state:', user['state'], ')', user);
      ctx.reply("Вы не можете пользоваться ботом, т.к. вы не является авторизованным пользователем")
    }
  });

  return [onTextMessage, bot, db] as const;
}